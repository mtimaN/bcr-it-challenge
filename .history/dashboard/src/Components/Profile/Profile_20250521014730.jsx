    ...
    <div className="profile-container">  
        ...
        <div className="general-information">
          ...
          <div className="search-icon-gender">
            ...
            <img
              src={isGenderOpen 
                ? (theme === 'light' ? upCollapseArrowDay : upCollapseArrowNight) 
                : (theme === 'light' ? downCollapseArrowDay : downCollapseArrowNight)}
              alt=""
            ...
            {isGenderOpen && (
              <div className="gender-dropdown-container">
                <div className="gender-selected-display">
                  <div className="gender-label">Gender</div>
                  <div className="gender-value">{selectedGender}</div>
                  <img 
                    src={theme === 'light' ? upCollapseArrowDay : upCollapseArrowNight}
                    alt=""
                    className="gender-arrow"
                  />
                </div>
                <ul className="gender-dropdown-menu">
                  {['Male', 'Female', 'Undisclosed'].map(option => (
                    <li
                      key={option}
                      className={`gender-dropdown-item ${selectedGender === option ? 'selected' : ''}`}
                      onClick={() => {
            ...
            {isMarriedOpen && (
              <ul className="married-dropdown-menu">
                {['Yes', 'No'].map(option => (
                  <li
                    key={option}
                    className={`married-dropdown-item ${selectedMarried === option ? 'selected' : ''}`}
                    onClick={() => {
